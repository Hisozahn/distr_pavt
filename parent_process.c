...
int parent_lifecycle(const pipe_io_ptr pipe_io, const pid_t *child_pids, int events_log_fd) {
	...
	for (int8_t i = 1; i < pipe_io->process_count; i++) {
		...
		advance_lamport_time(msg_started.s_header.s_local_time);
	}
	
	if (log_recieved_all_started(events_log_fd, get_lamport_time(), PARENT_ID) < 0) {
		return EXIT_FAILURE;
	}
	...
	for (int8_t i = 1; i < pipe_io->process_count; i++) {
		...
		advance_lamport_time(msg_done.s_header.s_local_time);
	}
	
	if (log_recieved_all_done(events_log_fd, get_lamport_time(), PARENT_ID) < 0) {
		return EXIT_FAILURE;
	}
